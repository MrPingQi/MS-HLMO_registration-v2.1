...
function descriptors = GGLOHv2_Descriptor(img, kps, ...
...
[WPMOM_m,WPMOM_o] = W_Parital_Main_Orientation(img,1,sqrt(rr1),4,int_flag);
% Weighted Partial Main Orientation Map, 取值范围：[0,pi)

if rot_flag
%     kps = [kps,diag(WPMOM_o(kps(:,2),kps(:,1)))];  % fast
    kps = Base_Direction(kps,WPMOM_m,WPMOM_o,W,NBO);  % normal
    sinth = sin(kps(:,end));
    costh = cos(kps(:,end));
else
...
for k = 1:size(kps,1)
    ...
    angle = WPMOM_o(y1:y2, x1:x2);  % 局部主方向图，取值范围：[0,pi)
    if rot_flag
    ...
    weight(W+y1-y+1:W+y2-y+1, W+x1-x+1:W+x2-x+1) = WPMOM_m(y1:y2, x1:x2);
